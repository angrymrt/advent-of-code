...
namespace day08
{
    ...
    {
        public int OutputValue => DecodeOutputValue();
        public string Value0Pattern => SignalPatterns.First(x => x.Length == 6 && x != Value9Pattern && x.Count(c => Value1Pattern.Contains(c)) == 2);
        public string Value1Pattern => SignalPatterns.First(x => x.Length == 2);
        public string Value2Pattern => SignalPatterns.First(x => x.Length == 5 && x != Value3Pattern && x != Value5Pattern);
        public string Value3Pattern => SignalPatterns.First(x => x.Length == 5 && x.Count(c => Value1Pattern.Contains(c)) == 2);
        public string Value4Pattern => SignalPatterns.First(x => x.Length == 4);
        public string Value5Pattern => SignalPatterns.First(x => x.Length == 5 && x != Value3Pattern && x.Count(c => Value6Pattern.Contains(c)) == 5);
        public string Value6Pattern => SignalPatterns.First(x => x.Length == 6 && x != Value0Pattern && x != Value9Pattern);
        public string Value7Pattern => SignalPatterns.First(x => x.Length == 3);
        public string Value8Pattern => SignalPatterns.First(x => x.Length == 7);
        public string Value9Pattern => SignalPatterns.First(x => x.Length == 6 && x.Count(c => Value4Pattern.Contains(c)) == 4);

        private int DecodeOutputValue()
        {
            var decodingTablet = new Dictionary<string, int>() {
                { Value0Pattern, 0 },
                { Value1Pattern, 1 },
                { Value2Pattern, 2 },
                { Value3Pattern, 3 },
            ...
            {
                stringAnswer += decodingTablet[output].ToString();
            }

            return int.Parse(stringAnswer);
        }

        public int UniqueOutputValueCount => OutputValues.Count(x => x.Length == 2 || x.Length == 3 || x.Length == 4 || x.Length == 7);
        private readonly string[] SignalPatterns;
        private readonly string[] OutputValues;
        ...
        static void Main(string[] args)
        {
            ...
            var decoders =
                (useTestInput ? testInput : input)
                    ...
                    .Select(x => new Decoder(x))
                    .ToArray();

            // Start part 1.
            sw.Start();
            answer = decoders.Sum(x => x.UniqueOutputValueCount);
            sw.Stop();
            ...
            // Start part 2.
            sw.Restart();
            answer = decoders.Sum(x => x.OutputValue);
            sw.Stop();

            Console.WriteLine("Answer to part 2: " + answer + " (" + sw.ElapsedMilliseconds + "ms, " + sw.ElapsedTicks + " ticks)");
        }

        public static string testInput = @"be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce";
        public static string input = @"defabc gcb dbafcg gc gcbed fbecgd begfdac fcbde cfge debag | gfce fgce bdefgca aebgd
