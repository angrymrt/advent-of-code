...
namespace day6
{
    class Group {
        ...
        public Group(string raw) {
            var set = new HashSet<char>(raw.Replace(Environment.NewLine, string.Empty));
            AnyoneAnsweredYes = set.Count;
            var persons = raw.Split(Environment.NewLine);
            var dictionary = new Dictionary<char, int>();
            foreach(var answers in persons) {
                foreach(var answer in answers){
                    if(!dictionary.TryAdd(answer, 1)) {
                        dictionary[answer] = dictionary[answer] + 1;
                    }
                }
            }
            
            EveryoneAnsweredYes = dictionary.Keys.Count(x => dictionary[x] == persons.Length);
        }

        public int AnyoneAnsweredYes { get; }
        public int EveryoneAnsweredYes { get; }
    }
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"Day 6, part 1 answer: {groups.Sum(x => x.AnyoneAnsweredYes)}");
            Console.WriteLine($"Day 6, part 2 answer: {groups.Sum(x => x.EveryoneAnsweredYes)}");
        }

        static private string input = @"qtmdwspah
sqwdamhpt
phwdaqsmt
stmdqwhap
pqawdhtms
