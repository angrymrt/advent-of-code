...
namespace day7
{
    class BagRule {
        ...
        public bool CanContainShinyGold(Dictionary<string, BagRule> rules) {
            var result = Contents.Any(x => x.Value.Color == ShinyGold || rules[x.Key].CanContainShinyGold(rules));
            return result;
        }
        public int CountTotalBagsInside(Dictionary<string, BagRule> rules) {
            var result = Contents.Sum(x => x.Value.Count + x.Value.Count * rules[x.Key].CountTotalBagsInside(rules));
            return result;
        }
        public BagRule(int count, string color){
            Count = count;
            Color = color;
            Contents = new Dictionary<string, BagRule>();
        }
        public BagRule(string raw){
            Color = raw.Split(" bags").First();
            Count = 1;
            if (raw.Contains("contain no other bags")) {
                Contents = new Dictionary<string, BagRule>();
            } else {
                Contents = raw.Split("bags contain ")
                                .Last()
                                .Replace(".", string.Empty)
                                .Replace(" bags", string.Empty)
                                .Replace(" bag", string.Empty)
                                .Split(", ")
                                .Select(x => new BagRule(int.Parse(x.Split(" ").First()), x.Split(" ").Skip(1).Aggregate((x,y)=>$"{x} {y}")))
        ...
        public Dictionary<string, BagRule> Contents { get; }
    }
    class Program
    {
        static void Main(string[] args)
        {
            var rules = input.Split(Environment.NewLine)
                            .AsParallel()
                            .Select(x => new BagRule(x))
                            .ToDictionary(x => x.Color);
            Console.WriteLine($"Day 7 answer, part 1: {rules.Count(x => x.Value.CanContainShinyGold(rules))}");
            Console.WriteLine($"Day 7 answer, part 2: {rules[BagRule.ShinyGold].CountTotalBagsInside(rules)}");
        }

        private static string input = @"muted tomato bags contain 1 bright brown bag, 1 dotted gold bag, 2 faded gray bags, 1 posh yellow bag.
