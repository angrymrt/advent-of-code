...
namespace day2
{
    public class CoporatePasswordPolicy {
        ...
        public CoporatePasswordPolicy(string text)
        {
            var split = text.Split(':');
            Password = split.Last().Substring(1);
            split = split.First().Split(' ');
            Letter = split.Last()[0];
            split = split.First().Split('-');
            MinCount = int.Parse(split.First());
            MaxCount = int.Parse(split.Last());
            var count = Password.Count(x => x == Letter);
            IsValid = MinCount <= count && MaxCount >= count;
            IsValidPartTwo = Password.Skip(MinCount-1).First() == Letter ^ Password.Skip(MaxCount-1).First() == Letter;
        }
    }
    class Program
    {
        ...
        {
            var policies = input.Select(x => new CoporatePasswordPolicy(x));

            Console.WriteLine($"Answer for day 2, part 1: {policies.Count(x => x.IsValid)}");
            Console.WriteLine($"Answer for day 2, part 2: {policies.Count(x => x.IsValidPartTwo)}");
        }

        static private string[] input = new string[] {
"1-7 j: vrfjljjwbsv",
"1-10 j: jjjjjjjjjjjj",
