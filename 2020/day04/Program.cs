...
namespace day4
{
    public class Passport {
        ...
        public bool IsValidPartTwo {get;private set;}
        public Passport(string data){
            var cleanData = data
                .Replace(Environment.NewLine, " ")
                .Split(' ')
                .Select(x => new KeyValuePair<string, string>(x.Substring(0, 3), x.Substring(4)));
            IsValid = cleanData.Count() == 8 || (cleanData.Count() == 7 && cleanData.Count(x => x.Key == "cid") == 0);

            if(IsValid){
                var dictionary = new Dictionary<string, string>(cleanData);
                // birth year validation..
                int byr = 0;
                if(!int.TryParse(dictionary["byr"], out byr)) return;
                if(byr < 1920 || byr > 2002) return;
                // issue year..
                int iyr = 0;
                if(!int.TryParse(dictionary["iyr"], out iyr)) return;
                if(iyr < 2010 || iyr > 2020) return;
                // expiration year..
                int eyr = 0;
                if(!int.TryParse(dictionary["eyr"], out eyr)) return;
                if(eyr < 2020 || eyr > 2030) return;
                // height year..
                int hgt = 0;
                var hgtVal = dictionary["hgt"].Substring(0, dictionary["hgt"].Length -2);
                var hgtUnit = dictionary["hgt"].Substring(dictionary["hgt"].Length -2);
                if(!int.TryParse(hgtVal, out hgt)) return;
                if(hgtUnit == "cm" && (hgt < 150 || hgt > 193)) return;
                if(hgtUnit == "in" && (hgt < 59 || hgt > 76)) return;
                if(hgtUnit != "cm" && hgtUnit != "in") return;
                // hair color
                var hexColorRegex = new Regex("^#[a-f0-9]{6}$");
                if(!hexColorRegex.IsMatch(dictionary["hcl"])) return;
                // eye color
                var eyeColor = dictionary["ecl"];
                var validEcls = new string[] { "amb", "blu", "brn", "gry", "grn", "hzl", "oth" };
                if(!validEcls.Contains(eyeColor)) return;
                // passport id
                var pidRegex = new Regex("^[0-9]{9}$");
                if(!pidRegex.IsMatch(dictionary["pid"])) return;
                // All check passed..
                IsValidPartTwo = true;
            }
        }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"Answer day 4, part 1: {passportData.Count(x => x.IsValid)}");
            Console.WriteLine($"Answer day 4, part 2: {passportData.Count(x => x.IsValidPartTwo)}");
        }

        private static string input = @"eyr:2033
hgt:177cm pid:173cm
